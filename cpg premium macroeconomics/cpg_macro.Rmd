...
```{r}
ggplot(data = consumption,aes(x=as.factor(year), y = rpce)) +
...
```{r}
ggplot(data = consumption,aes(x=as.factor(year), y = rpcend)) +
...
```{r}
ggplot(data = consumption,aes(x=as.factor(year), y = rpcedg)) +
  geom_bar(fill = "#4a9d36",stat="identity",width = .7) +
  geom_hline(yintercept = 0,lty = 2, col = "black") +
  ylab("Percent Change") +
  xlab("Year") +
...
```{r}
ggplot(data = consumption,aes(x=as.factor(year), y = rpces)) +
